...
def write_dashboard(payload: dict, path: str) -> str:
    ...
    nav = "".join(f'<a href="#{a}">{t}</a>' for a, t in [
        ("week", "This week"), ("why", "Why this pick"),
        ("sandbox", "Play the season"), ("teams", "Team detail"),
        ("compare", "Compare all 32"), ("board", "Full board"),
        ("schedule", "Schedule"), ("season", "Season so far"),
        ("method", "Method")])

    # ---- collapsible section bodies (built here so the page template below
    # stays a flat f-string) --------------------------------------------------
    sec_why = R.sec(
        "why", f"Why {R.esc(top['team'])}, and why now",
        '<p class="note">The no-reuse rule makes every team a single-use asset, '
        'so the question is never just "will they win" — it is "is this the week '
        'to spend them". Each number below is a counterfactual: the model '
        're-solves the whole season under a different assumption and reports '
        f'what changed.</p><div class="why">{why_cards}</div>', open_=True)

    sec_sandbox = R.sec(
        "sandbox", "Play the season out",
        '<p class="note">Answer a leg and the next one opens. Each answer '
        're-solves every remaining leg around it, so the equity you see is what '
        'that whole line is worth — not just this week. Remove any answer to '
        're-open everything below it. Best case if you follow the plan the whole '
        f'way is <b>{R.pct(payload["base_survival"], 3)}</b>.</p>'
        '<div id="wi-live" hidden></div>'
        '<details id="wi-static" open><summary>One table per leg</summary>'
        f'<div style="margin-top:10px">{R.ladder_static(payload)}</div>'
        '</details>', open_=True)

    sec_teams = R.sec(
        "teams", "Every team this week, in full",
        f'<p class="note">One card per team playing in '
        f'{R.esc(payload["leg_label"])}, in pick order. Tap any team to open '
        'every number the model has on them — this week\'s line and win '
        'probability, what picking them does to the season, where their value '
        'actually peaks, and how replaceable they are. Teams whose best slot is '
    ...
    sec_schedule = R.sec(
        ...
        'question the rest of the page cannot answer directly: who is even '
        'available in a given week, and when does a team you are holding '
        'actually play. Thanksgiving and Christmas are pulled out of Weeks 12 '
        'and 16 into their own columns, so a team playing on the holiday shows '
        'there and reads as a bye in the week column — which is exactly why '
        'those two legs have so few candidates.</p>'
        f'{R.schedule_table(payload)}')

    wi_data = json.dumps({
        "leg_key": payload["leg_key"],
        "leg_order": payload["leg_order"],
        "legs": [{"key": L["key"], "label": L["label"]} for L in payload["legs"]],
        "used": payload.get("used", []),
        "base_survival": payload["base_survival"],
...
<header>
  <div>
    ...
    <div class="sub">Leg {payload['leg_order']} of {n_legs} · updated
      {R.esc(payload['generated'])} · priced off moneylines ·
      {meta.get('n_moneylines', 0)} posted moneylines, {meta['n_lines']} spreads,
...
<details class="sec" id="board"><summary>
...
<details class="sec" id="season"><summary><h2>Season so far</h2>
  <span class="chev" aria-hidden="true"></span></summary><div class="secbody">
  <p class="note">The archive for <b>Survivor</b>, which is what this whole
  page is solved for, exactly as the model has always worked.
  Each completed leg is frozen here with the pick
...
<details class="sec" id="method"><summary><h2>How much to trust this</h2>
  <span class="chev" aria-hidden="true"></span></summary><div class="secbody">
  <p class="note">Where a moneyline is posted, the win probability is that
  price with the bookmaker's margin stripped out by Shin's method, which pulls
  more of the juice off the longshot than a simple proportional de-vig and so
  lands closer to the truth on heavy favourites. A moneyline pushes on a tie,
  so the de-vigged number is already "wins, given no tie" — the tie is then
  priced back in separately at {diag['tie_rate']*100:.2f}%, because in Circa a
  tie eliminates you. Where only a spread is posted, it is converted through an
  empirical margin model fit on {diag['holdout_n']:,}+ games of closing lines
  (margin sd {diag['sigma']}, held-out log loss {diag['holdout_logloss']}).
  Games with neither are projected from ridge-fit power ratings (home field
  {meta['home_field']}, scale correction ×{meta['proj_scale']}) and pulled
  toward a coin flip in proportion to how far out they are.</p>
  <div class="warn"><span>⚠</span><div><b>These are not Circa's prices.</b>
  Odds come from the nflverse feed, which is compiled consensus pricing across
  books. Circa publishes no public feed, so the numbers here track Circa
  closely but are not identical to what you will see on your ticket — check the
  actual price before you submit.</div></div>
...
<script>window.__DATA__ = {wi_data};</script>
<script>{JS}</script>
</body></html>"""
