...
from winprob import MarginModel, devig_moneyline
...
class Board:
    ...
    source: pd.DataFrame     # "ml", "line" or "proj"
    ml: pd.DataFrame         # American moneyline for the team, where priced
    meta: dict
...
def build_board(df: pd.DataFrame, season: int, mm: MarginModel,
    ...
    line_by_game = {}
    # A game that has already kicked off and finished is no longer a pick you
    # can make. Mid-week this matters: run the model on a Friday and Thursday's
    # teams are still sitting in the schedule with a perfectly good win
    # probability attached, and the optimizer will happily plan around an
    ...
    for _, g in sched.iterrows():
        ...
        if pd.notna(g.home_moneyline) and pd.notna(g.away_moneyline):
            ml_by_game[(g.home_team, g.away_team)] = (float(g.home_moneyline),
                                                      float(g.away_moneyline))
        if pd.notna(g.result):
            finished.add((g.home_team, g.away_team))
    ...
    ML = pd.DataFrame(np.nan, index=teams, columns=keys, dtype=float)
    ...
    for L in legs:
        ...
        for home, away, neutral in L.games:
            if (home, away) in finished:
                continue          # already played -- not selectable
            ...
            # A posted moneyline is a direct price on winning outright, which
            # is the only outcome this contest settles on. Prefer it over
            # translating a spread through the margin model -- fewer
            # assumptions between the market and the number. Moneylines push
            # on a tie, so scale back by the tie probability to get survival.
            mls = ml_by_game.get((home, away))
            if mls is not None:
                try:
                    fh, fa = devig_moneyline(*mls)
                    ptie = mm.p_tie(sp_home)
                    wh, wa = fh * (1 - ptie), fa * (1 - ptie)
                    src = "ml"
                    ML.loc[home, L.key], ML.loc[away, L.key] = mls[0], mls[1]
                except ValueError:
                    pass          # keep the spread-derived number
    ...
    meta["ratings"] = rt
    meta["hfa"] = hfa
    meta["n_moneylines"] = int(ML.notna().sum().sum() // 2)
    return Board(legs, teams, P, S, O, SRC, ML, meta)
...
def rank_picks(board: Board, current_key: str, remaining_keys: list[str],
    ...
    for t in available:
        ...
        rows.append({
            ...
            "moneyline": (None if pd.isna(board.ml.loc[t, current_key])
                          else int(board.ml.loc[t, current_key])),
    ...
    # Exact ties are common: two teams can be worth bit-identically the same
    # to the season, and the assignment solver may return either of two equally
    # optimal plans. Break ties on team abbreviation so the same inputs give
    # the same ordering on every machine -- otherwise comparing your run to
    # mine turns up phantom differences that are really just coin flips.
    out = pd.DataFrame(rows).sort_values(
        ["path_logp", "team"], ascending=[False, True])
